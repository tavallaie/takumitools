...
class CodeGenerator:
    ...
    def generate_code(self):
        for path, path_item in self.spec.get('paths', {}).items():
            nested_class_hierarchy = self._generate_nested_class_hierarchy(path, path_item)
    ...
    def _generate_nested_class_hierarchy(self, path: str, path_item: Dict[str, Any]):
        ...
        methods = [self._generate_method(http_method, path, op_details) for http_method, op_details in path_item.items()]
        ...
        for segment in path_segments:
            ...
            class_code += f"{indent}        self.factory = RequestHandlerFactory(client)\n\n"
            indent += '    '
    ...
    def _generate_method(self, http_method: str, path: str, operation: Dict[str, Any]):
        method_name = self._generate_method_name(operation.get('operationId', path), http_method)
        params_code, params_list, body_params = self._generate_params_code(operation.get('parameters', []), operation.get('requestBody'))
        ...
        if body_params:
            ...
            method_code += "}\n"
            method_code += "        body = {k: v for k, v in body.items() if v is not None}\n"
        else:
        ...
        if operation.get('requestBody'):
            for content_type in operation['requestBody']['content']:
                if content_type == 'application/json':
                    method_code += "        headers['Content-Type'] = 'application/json'\n"
                elif content_type == 'multipart/form-data':
                    method_code += "        headers['Content-Type'] = 'multipart/form-data'\n"
        
        if any(p['in'] == 'query' for p in params_list):
            query_params = ', '.join([f"'{param['name']}': {param['name']}" for param in params_list if param['in'] == 'query'])
            method_code += f"        params = {{ {query_params} }}\n"
            method_code += "        params = {k: v for k, v in params.items() if v is not None}\n"
        else:
    ...
    def _generate_params_code(self, parameters: List[Dict[str, Any]], request_body: Optional[Dict[str, Any]]):
        params_code = []
        params_list = []
        body_params = []
        for param in parameters:
            ...
            params_list.append({'name': param_name, 'in': param_in, 'description': param_desc})
            annotation = f": {param_type}" if required else f": Optional[{param_type}] = None"
            params_code.append(f"{param_name}{annotation}")
        
        if request_body:
            for prop_name, prop_details in request_body['content']['application/json']['schema']['properties'].items():
                param_type = self._get_param_type(prop_details)
                required = prop_name in request_body['content']['application/json']['schema'].get('required', [])
                annotation = f": {param_type}" if required else f": Optional[{param_type}] = None"
